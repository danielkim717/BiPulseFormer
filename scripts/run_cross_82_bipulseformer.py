"""Cross-dataset BiPulseFormer — 8:2 protocol with full target test.

PURE -> UBFC-rPPG:
  TRAIN (PURE 0.0-0.8): 80% of PURE subjects (subject-exclusive)
  VALID (PURE 0.8-1.0): 20% of PURE subjects (best-epoch 선택)
  TEST  (UBFC ENTIRE):  100% of UBFC-rPPG

UBFC-rPPG -> PURE:
  TRAIN (UBFC 0.0-0.8): 80% of UBFC subjects
  VALID (UBFC 0.8-1.0): 20% of UBFC subjects
  TEST  (PURE ENTIRE):  100% of PURE

Setup (Intra Protocol 2 와 동일):
  - OneCycleLR(max_lr=1e-4, epochs=20, per-batch step)
  - 20 epochs
  - Loss schedule: epoch≤10 a=1.0, b=1.0;  epoch>10 a=0.05, b=5.0
  - Per-subject + per-clip 모두 보고 (paper-comparable)
"""
...
def run_cross(src_name, src_path, tgt_name, tgt_path, result_dir):
    ...
    log("=" * 70); log(f"[*] {label}  (cross 8:2, full target test)"); log("=" * 70)
    ...
    for epoch in range(EPOCHS):
        ...
        log(f"  TEST  per-clip MAE {test_m['MAE_clip']:.3f}  RMSE {test_m['RMSE_clip']:.3f}  "
            f"MAPE {test_m['MAPE_clip']:.3f}%  Pearson {test_m['Pearson_clip']:.4f}  (n_clips={test_m['n_clips']})")
    ...
    log(f"   per-clip  MAE {best_test['MAE_clip']:.3f}  RMSE {best_test['RMSE_clip']:.3f}  "
        f"MAPE {best_test['MAPE_clip']:.3f}%  Pearson {best_test['Pearson_clip']:.4f}")
...
def main():
    ...
    print(f"[*] Cross 8:2 BiPulseFormer (OneCycleLR + 20 epochs)")
    results = []
    for src, srp, tgt, tgp, out in [
        ('PURE', PURE_PATH, 'UBFC-rPPG', UBFC_PATH, 'results/cross_82_pure_to_ubfc'),
        ('UBFC-rPPG', UBFC_PATH, 'PURE', PURE_PATH, 'results/cross_82_ubfc_to_pure'),
    ]:
        try:
    ...
    print("\n" + "=" * 80)
    print("[*] Final cross 8:2 — per-subject (paper-comparable)")
    print("=" * 80)
