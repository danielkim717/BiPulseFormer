...
기존 run_cross_82_const30.py 대비 변경:
  - OneCycleLR → StepLR(step_size=50, gamma=0.5) [사실상 constant LR for 15 epochs]
  - EPOCHS: 30 → 15 (best epoch 7 부근에서 도달했으므로 충분)
  - 학습 시간 절약: 25시간 → 12시간 (UBFC→PURE 기준)

  근거: results/hyperparameter_recommendations.md
    - 30 epoch OneCycleLR 셋업이 epoch 7 best 이후 21 epoch source overfit
    - StepLR + 15 epoch 가 페이퍼/rPPG-Toolbox 표준 + 더 안정적

기본 동작: UBFC-rPPG → PURE 만 학습 (PURE→UBFC 는 이미 완료, E7 best)
--all 옵션으로 양방향 모두 학습 가능.
"""
...
LR = 1e-4
WD = 5e-5
STEP_SIZE = 50             # StepLR(50, 0.5) → 15 epoch 동안 LR=1e-4 constant
GAMMA = 0.5
ALPHA = 1.0
BETA = 1.0
GRA_SHARP = 2.0
...
def run_cross(src_name, src_path, tgt_name, tgt_path, result_dir):
    ...
    log("=" * 70)
    log(f"[*] {label}  (StepLR + 15ep, const α=β=1)")
    log("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(description='Cross 8:2 BiPulseFormer (StepLR + 15ep, const α=β=1)')
    parser.add_argument('--direction', choices=['ubfc2pure', 'pure2ubfc', 'both'],
                       default='ubfc2pure',
                       help='기본: ubfc2pure (PURE→UBFC 는 이미 완료)')
    args = parser.parse_args()
    _seed_everything(SEED)
    print(f"[*] Cross 8:2 BiPulseFormer (StepLR + 15ep, const α=β=1)  direction={args.direction}")

    experiments = []
    if args.direction in ('pure2ubfc', 'both'):
        experiments.append(('PURE', PURE_PATH, 'UBFC-rPPG', UBFC_PATH,
                           'results/cross_82_pure_to_ubfc_steplr15'))
    if args.direction in ('ubfc2pure', 'both'):
        experiments.append(('UBFC-rPPG', UBFC_PATH, 'PURE', PURE_PATH,
                           'results/cross_82_ubfc_to_pure_steplr15'))
    ...
    print("\n" + "=" * 80)
    print("[*] Final cross 8:2 steplr15 — per-subject (paper-comparable)")
    print("=" * 80)
