...
RESULT_DIR = 'results/intra_biphysformer'
LOG = os.path.join(RESULT_DIR, 'log.txt')
LIVE_FILE = os.path.join('results', 'live_progress_intra.md')
STATUS_FILE = os.path.join('results', 'current_status_intra.json')
...
def run_experiment(train_name, train_path, test_name, test_path):
    ...
    # Intra-dataset subject-exclusive split (PhysFormer paper protocol):
    #   - Train: 0-60% subjects, RandomHorizontalFlip + HR filter
    #   - Valid: 60-80% subjects, best-epoch selection
    #   - Test:  80-100% subjects (NEVER seen, fair report)
    # rPPG-Toolbox config 동일 (BEGIN/END 0.0-0.6/0.6-0.8/0.8-1.0)
